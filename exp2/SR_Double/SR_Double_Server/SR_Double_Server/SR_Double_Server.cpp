...
const int BUFFER_LENGTH = 1024;    //缓冲区大小，（以太网中 UDP 的数据帧中包长度应小于 1480 字节） 
const int SEND_WIND_SIZE = 10;//发送窗口大小为 10，GBN 中应满足  W + 1 <= N（W 为发送窗口大小，N 为序列号个数）
//本例取序列号 0...19 共 20 个 
//如果将窗口大小设为 1，则为停-等协议 

const int SEQ_SIZE = 200; //序列号的个数，从 0~19 共计 20 个 
...
void getCurTime(char* ptime) {
	...
	time(&c_time);
	p = localtime(&c_time);
	sprintf_s(buffer, "%2d/%2d/%2d %2d:%2d:%2d",
		p->tm_year + 1900,
		p->tm_mon+1,
		p->tm_mday,
		p->tm_hour,
...
int main(int argc, char* argv[])
{
	...
	if (err != 0) {
		//找不到 winsock.dll 
		printf("WSAStartup failed with error: %2d\n", err);
		return -1;
	}
	...
	if (err) {
		err = GetLastError();
		printf("Could  not  bind  the  port  %2d  for  socket.Error  code is %2d\n", SERVER_PORT, err);
		WSACleanup();
		return -1;
	}
	...
	while (true) {
		...
		if (strcmp(buffer, "-time") == 0) {
			...
			out_result.open("../../server_receive.txt", std::ios::out | std::ios::trunc);
				...
				case 1://等待接收 200 阶段，没有收到则计数器+1，超时则放弃此次“连接”，等待从第一步开始
					...
					if (recv_lvxy < 0) {
						...
						if ((unsigned char)buffer[0] == 200) {
							...
							printf("File size is %2dB, each packet is 1024B and packet total num is %2d\n", sizeof(data), totalPacket);
							curSeq = 0;
							curAck = 0;
							totalSeq = 0;
							waitCount = 0;
				...
				case 2://数据传输阶段 					
					if (seqIsAvailable() && totalSeq - SEND_WIND_SIZE <= totalPacket) {
						...
						/*if (!recvSeq){
							...
							buffer[0] = curSeq + 1;
							if (b){
								printf("The  packet from client  of  %2d  loss\n", (unsigned char)buffer[0]);
								break;
							}
							...
							printf("SERVER：send a packet with a seq of %2d and a ack of %2d\n", curSeq + 1, seq);
						...
						//	printf("%2d", WSAGetLastError());
							...
							if (b){
								printf("The packet from Client with a seq of %2d and ACK for SERVER of %2d loss\n", buffer[0], buffer[1]);
								break;
							}
							seq = (unsigned short)buffer[0];

							printf("recv packet: %2d, ACK1 for SERVER:%2d\n", seq, (unsigned short)buffer[1]);
							...
							b = lossInLossRatio(ackLossRatio);
							if (b){
								printf("The  packet from Client  of  %2d and ACK for server of %2d loss\n", (unsigned char)buffer[0], seq);
								break;
							}
							ack[curSeq] = 0;
							//数据发送的过程中应该判断是否传输完成
							//为简化过程此处并未实现
							memcpy(&buffer[2], data + 1024 * (curSeq + (totalSeq / SEND_WIND_SIZE)*SEND_WIND_SIZE), 1024);
							printf("send a packet from Client  of  %2d and ACK for server of %2d \n", curSeq + 1, seq);
							sendto(sockServer, buffer, BUFFER_LENGTH, 0,
								(SOCKADDR*)&addrClient, sizeof(SOCKADDR));
