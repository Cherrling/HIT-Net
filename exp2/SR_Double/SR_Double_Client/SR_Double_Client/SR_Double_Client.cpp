...
const int SEQ_SIZE = 200;//接收端序列号个数，为 1~20 
...
void printTips() {
	printf("************************************************************\n");
	printf("-time 获取当前时间    \n");
	printf("-test X Y to test SR\n");
	printf("X为包丢失率 Y为ACK丢失率\n");
	printf("-quit 退出\n");
}
//************************************ 
...
int main(int argc, char* argv[])
{
	...
	icin.close();
	int handle;
	handle = _open("../../client_origin.txt", 0x0100); //open file for read
	...
	//使用  -test [X] [Y]  测试 GBN  其中[X]表示数据包丢失概率 
	//                    [Y]表示 ACK 丢包概率 
	printTips();
	...
	while (true) {
		...
		if (!strcmp(cmd, "-test")) {
			...
			out_result.open("../../client_receive.txt", std::ios::out | std::ios::trunc);
			if (!out_result.is_open()) {
				printf("文件打开失败！！！\n");
				continue;
			}
			//---------------------------------
			sendto(socketClient, "-test", strlen("-test") + 1, 0,
